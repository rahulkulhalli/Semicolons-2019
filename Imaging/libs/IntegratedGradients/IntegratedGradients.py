...
class smoothed_integrated_gradients(integrated_gradients):

    # def __init__(self, model, outchannels=[], verbose=1):
    #     super(smoothed_integrated_gradients, self).__init__(model ,outchannels, verbose)

    def explain_smooth(self, sample, outc=0, reference=False, num_steps_integrad=50, verbose=0,
                nsamples=25, local_smoothing=0.15, seed=100, magnitude=True):
        # Set seed every time to ensure consistent results across runs
        np.random.seed(seed)
        ...
        stdev = (np.max(sample) - np.min(sample)) * local_smoothing
        total_gradient = np.zeros(sample.shape)

        for i in range(nsamples):
            sys.stdout.write('\r')
            sys.stdout.write("Progress: {} / {}".format(i, nsamples))
            sys.stdout.flush()
            x_plus_noise = sample + np.random.normal(0., stdev, sample.shape)
            grad = self.explain(x_plus_noise, outc, reference, num_steps_integrad, verbose)
            if magnitude:
                total_gradient += (grad * grad)
            else:
                total_gradient += grad
        total_gradient /= nsamples
